...
public class MainWeatherFrame extends javax.swing.JFrame {
    ...
    public MainWeatherFrame(String location) {
        this.userLocation = location; // Assign the input location to the member variable
        ...
        LocationLabel.setText(userLocation); // Use the userLocation member variable

        String weatherData = getWeatherData(userLocation); // Fetch weather data using the userLocation
        if (weatherData != null) {
    ...
    private void RefreshIconLabelMouseClicked(MouseEvent evt) {  
    	String weatherData = getWeatherData(entry.getLocation());
        if (weatherData != null) {
            parseWeatherData(weatherData);
        }
    ...
    public String getWeatherData(String location) {
        try {
            String apiKey = "a19c55c36ed66d550c2cce7bdf03eb99";
            String encodedLocation = URLEncoder.encode(location, "UTF-8"); // Encode the location
            String urlString = "http://api.openweathermap.org/data/2.5/weather?q=" + encodedLocation + "&appid=" + apiKey;
 ...
 public void parseWeatherData(String jsonData) {
        ...
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                CurrentTempLabel.setText(String.format("%.2f °C", tempInCelsius));
                DescriptionLabel.setText(weather);
    ...
    public static void main(String args[]) {
       /* BlockUI check = new BlockUI();
    	...
    	String location = JOptionPane.showInputDialog(null, "Enter a location for a weather forecast:", "Location Input", JOptionPane.PLAIN_MESSAGE);
        
        if (location != null && !location.isEmpty()) {
            // Assuming FeedEntry can take a location as a parameter
            // Update this part according to how FeedEntry should be initialized
            FeedEntry entry = new FeedEntry(location); 
            EventQueue.invokeLater(new Runnable() {
                public void run() {
                    new MainWeatherFrame(entry);
                }
